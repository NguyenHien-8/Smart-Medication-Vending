...
Json Parse(const std::string& text) {
    return Json(cJSON_ParseWithLengthOpts(text.c_str(), text.size() + 1, nullptr, true),
                &cJSON_Delete);
}
Json Make() { return Json(cJSON_CreateObject(), &cJSON_Delete); }
const cJSON* Get(const cJSON* o, const char* key) {
...
void AddText(cJSON* a, const std::string& v) {
    cJSON_AddItemToArray(a, cJSON_CreateString(v.c_str()));
}
std::string Serialize(const cJSON* j) {
    if (!j)
        return "{\"status\":\"DENY\",\"vend_allowed\":false}";
...
std::set<std::string> ReadEnumArray(const cJSON* value, bool& valid, size_t max = 24) {
    ...
    cJSON_ArrayForEach (entry, value) {
        ...
        if (!s || !*s || std::strlen(s) > 64 || !result.insert(s).second)
            valid = false;
    }
    return result;
}
...
MedicalEvaluation CompleteEvaluation(const cJSON* out, MedicalDecision decision,
                                     std::optional<MedicalOffer> offer = std::nullopt) {
    MedicalEvaluation result;
    result.decision = decision;
    if (const char* reason = Str(Get(out, "reason")))
        result.reason = reason;
    if (const char* session = Str(Get(out, "session_id")))
        result.session_id = session;
    if (const cJSON* turn = Get(out, "turn_id"); cJSON_IsNumber(turn) && turn->valuedouble >= 0)
        result.turn_id = static_cast<uint32_t>(turn->valuedouble);
    result.offer = std::move(offer);
    result.response_json = Serialize(out);
    return result;
}
MedicalEvaluation EvaluationError(const char* reason) {
    auto out = Make();
    Put(out.get(), "status", "DENY");
    Put(out.get(), "reason", reason);
    cJSON_AddBoolToObject(out.get(), "vend_allowed", false);
    return CompleteEvaluation(out.get(), MedicalDecision::kDeny);
}
...
void NextField(cJSON* out, const char* field) {
    ...
    for (const auto& q : kQuestions)
        if (std::strcmp(field, q.key) == 0) {
...
const cJSON* FindCheck(const cJSON* interview, const char* id) {
    ...
    cJSON_ArrayForEach (check, Get(interview, "global_checks")) {
        if (const char* key = Str(Get(check, "id")); key && std::strcmp(id, key) == 0)
            return check;
    }
...
bool IsPositiveDanger(const cJSON* interview, const cJSON* answers) {
    ...
    cJSON_ArrayForEach (a, answers) {
        ...
        if (check && cJSON_IsTrue(a) && cJSON_IsBool(Get(check, "expected")) &&
            !cJSON_IsTrue(Get(check, "expected")) &&
            std::strcmp(Str(Get(check, "on_mismatch")) ? Str(Get(check, "on_mismatch")) : "",
                        "REFER") == 0)
            return true;
    }
    return false;
...
const cJSON* FindSlot(const cJSON* catalog, const char* canonical, bool backup) {
    ...
    cJSON_ArrayForEach (slot, slots) {
        ...
        const bool is_backup = cJSON_IsNumber(Get(slot, "backup_of_channel")) != 0;
        if (id && std::strcmp(id, canonical) == 0 && is_backup == backup)
            return slot;
    }
    return nullptr;
}
...
std::string MedicalAdvisor::IntakeSchema() const {
    ...
    Put(out.get(), "instruction",
        "Describe the user's symptoms without assuming mildness or a diagnosis. Call "
        "evaluate_symptoms after EACH user reply using only facts explicitly stated. Ask only "
        "next_question_vi, with no preface or extra question. Unclear, incomplete or unrelated "
        "reply: repeat the pending question without adding an answer. Unknown fields are OMITTED, "
        "never false or []. Never suggest a medicine before a device result.");
    ...
    static constexpr struct {
        const char* id;
        const char* vi;
    } kLabels[] = {{"fever", "sốt đo nhiệt độ"},
                   {"mild_headache", "đau đầu nhẹ; chưa rõ mức độ thì hỏi lại"},
                   {"mild_body_ache", "đau mỏi nhẹ"},
                   {"mild_inflammatory_pain", "đau cơ hay khớp nhẹ"},
                   {"allergic_rhinitis", "hắt hơi, ngứa mũi/mắt do dị ứng"},
                   {"dry_cough", "ho không đờm"},
                   {"productive_cough", "ho có đờm"},
                   {"mild_sore_throat", "đau rát họng nhẹ"},
                   {"gas_bloating", "đầy hơi, ợ hơi"},
                   {"acid_indigestion", "ợ nóng, khó tiêu acid"},
                   {"short_term_reflux", "ợ chua, trào ngược"},
    ...
    for (const auto& item : kLabels) {
        if (supported.count(item.id))
            Put(groups, item.id, item.vi);
    }
    if (cJSON_GetArraySize(groups) != static_cast<int>(supported.size()))
        return Error("DENY", "UNMAPPED_SYMPTOM");
    Put(out.get(), "unknown_semantics",
        "Omit unknowns. [] only after explicitly asked and denied; 'no' answers ONLY the one "
        "pending question.");
...
std::string MedicalAdvisor::SymptomGuide(const std::string& symptom_enum) const {
    ...
    if (!rules || !catalog)
        return Error("DENY", "LOCAL_DATA_UNAVAILABLE");
    const auto supported = AllowedSymptoms(rules.get());
    if (!supported.count(symptom_enum))
        return Error("DENY", "UNKNOWN_SYMPTOM");
    const auto* checks =
        Get(Get(Get(rules.get(), "interview"), "symptom_checks"), symptom_enum.c_str());
    ...
    cJSON_ArrayForEach (rule, Get(rules.get(), "medicine_rules")) {
        ...
        if (!canonical || !HasAny(std::set<std::string>{symptom_enum}, Get(rule, "symptoms")))
            continue;
    ...
    if (!slot)
        return Error("DENY", "NO_SYMPTOM_GUIDE");
    if (const char* cue = Str(Get(slot, "symptom_group")))
        Put(out.get(), "symptom_cues_vi", cue);
    if (const char* caution = Str(Get(slot, "differentiate_and_escalate")))
        Put(out.get(), "do_not_confuse_with_vi", caution);
    Put(out.get(), "instruction",
        "Use to clarify symptom classification only; do not read all checks in one turn. Actual "
        "next question and any provisional option MUST come from evaluate_symptoms.");
    cJSON_AddBoolToObject(out.get(), "vend_allowed", false);
    const std::string response = Serialize(out.get());
    return response.size() <= 2400 ? response : Error("DENY", "SYMPTOM_GUIDE_TOO_LARGE");
}

MedicalEvaluation MedicalAdvisor::EvaluateStructured(const std::string& untrusted_json) const {
    if (untrusted_json.size() > 4096)
        return EvaluationError("INPUT_TOO_LARGE");
    ...
    if (!rules || !catalog || !review || !cJSON_IsObject(rules.get()) ||
        ...
        return EvaluationError("LOCAL_DATA_UNAVAILABLE");
    ...
    if (!cJSON_IsBool(Get(review.get(), "approved")))
        return EvaluationError("INVALID_PHARMACIST_REVIEW_CONFIGURATION");
    ...
    if (!cJSON_IsNumber(min_age) || !cJSON_IsNumber(max_meds) || !rule_version ||
        !catalog_version || !cJSON_IsArray(required) ||
        !cJSON_IsArray(Get(rules.get(), "medicine_rules")) ||
        !cJSON_IsArray(Get(catalog.get(), "slots")) ||
        !cJSON_IsArray(Get(rules.get(), "global_danger_signs")) ||
        !cJSON_IsObject(Get(rules.get(), "interview")) ||
        !cJSON_IsArray(Get(Get(rules.get(), "interview"), "global_checks")) ||
        !cJSON_IsObject(Get(Get(rules.get(), "interview"), "symptom_checks")))
        return EvaluationError("INVALID_LOCAL_CONFIGURATION");
    auto input = Parse(untrusted_json);
    if (!input || !cJSON_IsObject(input.get()))
        return EvaluationError("INVALID_INPUT_JSON");
    // Never accept cloud-side authorization, channel, SKU, quantity, or a free-text prescription.
    static const std::set<std::string> kFields = {"session_id",
                                                  "turn_id",
                                                  "age_years",
                                                  "weight_kg",
    ...
    cJSON_ArrayForEach (entry, input.get()) {
        if (!entry->string || !kFields.count(entry->string))
            return EvaluationError("FORBIDDEN_OR_UNKNOWN_FIELD");
        if (!seen_fields.insert(entry->string).second)
            return EvaluationError("DUPLICATE_FIELD");
    }
    ...
    if (answers) {
        if (!cJSON_IsObject(answers) || cJSON_GetArraySize(answers) > 48)
            return EvaluationError("INVALID_SCREENING_ANSWERS");
    ...
    if (!session || !*session || std::strlen(session) > 64 || !cJSON_IsNumber(turn) ||
        ...
        return EvaluationError("INVALID_SESSION_OR_TURN");
    auto out = Make();
    ...
    if (age && (!cJSON_IsNumber(age) || !std::isfinite(age->valuedouble) ||
                std::floor(age->valuedouble) != age->valuedouble || age->valuedouble < 0 ||
                age->valuedouble > 120))
        return EvaluationError("INVALID_AGE");
    if (age && age->valuedouble < min_age->valuedouble) {
        Put(out.get(), "status", "REFER");
        Put(out.get(), "reason", "UNDER_MINIMUM_AGE");
        return CompleteEvaluation(out.get(), MedicalDecision::kRefer);
    }
    const auto* pregnancy = Get(input.get(), "pregnancy_or_breastfeeding");
    if (pregnancy && !cJSON_IsBool(pregnancy))
        return EvaluationError("INVALID_PREGNANCY_FIELD");
    if (cJSON_IsTrue(pregnancy)) {
        Put(out.get(), "status", "REFER");
        Put(out.get(), "reason", "PREGNANCY_OR_BREASTFEEDING_UNSUPPORTED");
        return CompleteEvaluation(out.get(), MedicalDecision::kRefer);
    }
    bool valid = true;
    std::set<std::string> danger, symptoms, conditions, medicines, allergies;
    if (Get(input.get(), "danger_signs"))
        danger = ReadEnumArray(Get(input.get(), "danger_signs"), valid);
    if (Get(input.get(), "symptoms"))
        symptoms = ReadEnumArray(Get(input.get(), "symptoms"), valid);
    if (Get(input.get(), "conditions"))
        conditions = ReadEnumArray(Get(input.get(), "conditions"), valid);
    if (Get(input.get(), "current_medicines"))
        medicines = ReadEnumArray(Get(input.get(), "current_medicines"), valid);
    if (Get(input.get(), "drug_allergies"))
        allergies = ReadEnumArray(Get(input.get(), "drug_allergies"), valid);
    if (!valid)
        return EvaluationError("INVALID_ARRAY_OR_DUPLICATE_FLAG");
    ...
    cJSON_ArrayForEach (check, global_checks) {
        ...
        if (!id || !Str(Get(check, "question_vi")) || !cJSON_IsBool(Get(check, "expected")))
            return EvaluationError("INVALID_INTERVIEW_CONFIG");
        ...
        if (cJSON_IsTrue(answer) != cJSON_IsTrue(Get(check, "expected"))) {
            ...
            if (!mismatch ||
                (std::strcmp(mismatch, "REFER") != 0 && std::strcmp(mismatch, "CLARIFY") != 0))
                return EvaluationError("INVALID_INTERVIEW_CONFIG");
            ...
            return CompleteEvaluation(out.get(), std::strcmp(mismatch, "REFER") == 0
                                                     ? MedicalDecision::kRefer
                                                     : MedicalDecision::kAsk);
        }
    }
    ...
    if (cJSON_GetArraySize(missing) > 0) {
        ...
        if (const char* key = Str(first))
            NextField(out.get(), key);
        return CompleteEvaluation(out.get(), MedicalDecision::kAsk);
    }
    ...
    if (!cJSON_IsNumber(weight) || !std::isfinite(weight->valuedouble) ||
        weight->valuedouble <= 0 || weight->valuedouble > 300 || !cJSON_IsNumber(duration) ||
        !std::isfinite(duration->valuedouble) || duration->valuedouble < 0 ||
        duration->valuedouble > 24 * 365)
        return EvaluationError("INVALID_WEIGHT_OR_DURATION");
    if (symptoms.empty()) {
        Put(out.get(), "status", "NEED_MORE_INFO");
        Put(out.get(), "reason", "SYMPTOMS_NOT_SPECIFIED");
        return CompleteEvaluation(out.get(), MedicalDecision::kAsk);
    ...
    if (weight->valuedouble < 50)
        combined.insert("weight_below_50kg");
    if (age->valuedouble < 18)
        combined.insert("age_16_or_17");
    if (duration->valuedouble > 48)
        combined.insert("duration_over_48_hours");
    ...
    for (const auto& symptom : symptoms) {
        ...
        if (!cJSON_IsArray(questions) || cJSON_GetArraySize(questions) == 0)
            return EvaluationError("MISSING_SYMPTOM_INTERVIEW_CONFIG");
        ...
        cJSON_ArrayForEach (detail, questions) {
            ...
            if (!id || !question || !cJSON_IsBool(expected) || !action ||
                (std::strcmp(action, "REFER") != 0 && std::strcmp(action, "CLARIFY") != 0))
                return EvaluationError("INVALID_INTERVIEW_CONFIG");
            const cJSON* answer = Get(answers, id);
            if (!answer) {
            ...
            if (cJSON_IsTrue(answer) != cJSON_IsTrue(expected)) {
                ...
                return CompleteEvaluation(out.get(), std::strcmp(action, "REFER") == 0
                                                         ? MedicalDecision::kRefer
                                                         : MedicalDecision::kAsk);
            }
        }
    ...
    bool referred = false;
    std::optional<MedicalOffer> medical_offer;
    const cJSON* rule = nullptr;
    cJSON_ArrayForEach (rule, Get(rules.get(), "medicine_rules")) {
        if (!HasAny(symptoms, Get(rule, "symptoms")))
            continue;
        const char* canonical = Str(Get(rule, "canonical_id"));
        if (!canonical)
            return EvaluationError("INVALID_RULE_ID");
        ...
        if (!main || !cJSON_IsNumber(Get(main, "channel")))
            return EvaluationError("INVALID_MAIN_CATALOG_SLOT");
        const char* name = Str(Get(main, "name"));
        const char* strength = Str(Get(main, "strength"));
        const char* ingredient = Str(Get(main, "active_ingredient"));
        if (!name || !strength || !ingredient)
            return EvaluationError("INVALID_CATALOG_ITEM");
        ...
        ++eligible_count;
        if (count >= 1 || count >= max_meds->valueint)
            continue;
        ...
        medical_offer = MedicalOffer{canonical, name, ingredient, strength};
        ++count;
    }
    if (eligible_count > 1) {
        ...
        Put(out.get(), "status", "REFER");
        Put(out.get(), "reason",
            referred ? "RULE_EXCLUSION_OR_REFERRAL" : "NO_REVIEWABLE_OPTION_IN_CATALOG");
    }
    Put(out.get(), "safety_notice",
        "AI-reported facts are unverified. No diagnosis, dose, dispense authorization, or relay "
        "operation.");
    return CompleteEvaluation(out.get(),
                              medical_offer ? MedicalDecision::kOffer : MedicalDecision::kRefer,
                              std::move(medical_offer));
}

std::string MedicalAdvisor::Evaluate(const std::string& untrusted_json) const {
    return EvaluateStructured(untrusted_json).response_json;
}
}  // namespace smv
